defmodule Volta.Testing.LND do
  use GenServer
  ...
  defp download(state) do
    ...
    tar_url = "https://github.com/lightningnetwork/lnd/releases/download/#{version}/lnd-linux-amd64-#{version}.tar.gz"
  ...
  def init(opts) do
    ...
    download(state)
    port = Port.open({:spawn, "/src/resources/safe_start.sh #{state[:lnd_path]} #{lnd_args(state)}"}, [:binary])
    state = state
    |> Map.put(:port, port)
  ...
  defp init_wallet(state) do
    ...
    """
    {:ok, resp} = HTTPoison.post("https://127.0.0.1:#{state[:rest_port]}/v1/initwallet", json, [], [ssl: [verify: :verify_none]])
