defmodule Lnx.Onion do
  alias Lnx.Key

  defmodule OnionPacketV0 do
    alias Lnx.Onion.PerHop
    @num_max_hops 20
    @hop_data_size 65
    ...
    def unwrap(packet, key) do
      ...
      hop_shared_secret = :crypto.hash(:sha256, Key.compress(ecdh_result))

      blinding_factor = :crypto.hash(:sha256, Key.compress(ephem_pub_key) <> hop_shared_secret)
    end
    ...
    def create_with_intermediates(payment_path, session_key, associated_data) do
      {reverse_hop_shared_secrets, ephem_key, blinding_factors, ephem_keys} =
        ...
        |> Enum.reduce({[], session_key, [], []}, fn hop_pub_key,
                                                     {hop_shared_secrets, ephem_key, bfs, eks} ->
          {:ok, ecdh_result} = :libsecp256k1.ec_pubkey_tweak_mul(hop_pub_key, ephem_key)
          hop_shared_secret = :crypto.hash(:sha256, Key.compress(ecdh_result))

          ephem_pub_key = Key.pub_from_priv(ephem_key)

          blinding_factor =
            :crypto.hash(:sha256, Key.compress(ephem_pub_key) <> hop_shared_secret)
      ...
      {last_hmac, mix_header, _, rho_keys, mu_keys, plain_routing_infos, encrypted_routing_infos,
       ...
       hmacs} =
        ...
        |> Enum.reduce(
          ...
          fn {i, hop_pub_key, hop_payload, shared_secret},
             {last_hmac, mix_header, hmac, rho_keys, mu_keys, pri, eri, hmac_datas, hmacs} ->
            rho_key = generate_key("rho", shared_secret)
            ...
            mix_header =
              hop_data <> binary_part(mix_header, 0, @routing_info_size - @hop_data_size)
      ...
      packet = %OnionPacketV0{
        version: 0,
        public_key: Key.pub_from_priv(session_key) |> Key.compress(),
  ...
  def parse_packet(
        0,
        <<public_key::bytes-size(33), hops_data::bytes-size(1300), hmac::bytes-size(32)>>
      ) do
  end
