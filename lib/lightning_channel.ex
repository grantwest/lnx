defmodule Lnx.LightningChannel do
  @moduledoc """
  ...
  defp open_msg(%{opts: opts, channel_id: ch_id} = ch) do
    ...
    first_per_commitment_point = Key.pub_from_priv(commitment_secret)
  ...
  defp validate(ch, %OpenChannelMsg{} = msg) do
    cond do
      msg.push_msat > 1000 * msg.funding_satoshis ->
        {:invalid,
         "push (#{msg.push_msat} msat) cannot be greater than funding (#{msg.funding_satoshis} sat)"}
      ...
      msg.dust_limit_satoshis > msg.channel_reserve_satoshis ->
        {:invalid,
         "dust limit (#{msg.dust_limit_satoshis} sat) cannot be greater than reserve (#{msg.channel_reserve_satoshis} sat)"}

      !Key.valid_public_key?(msg.funding_pubkey) ->
        {:invalid, "funding_pubkey (#{inspect(msg.funding_pubkey)}) is not a valid public key"}

      !Key.valid_public_key?(msg.revocation_basepoint) ->
        {:invalid,
         "revocation_basepoint (#{inspect(msg.revocation_basepoint)}) is not a valid public key"}

      !Key.valid_public_key?(msg.payment_basepoint) ->
        {:invalid,
         "payment_basepoint (#{inspect(msg.payment_basepoint)}) is not a valid public key"}

      !Key.valid_public_key?(msg.delayed_payment_basepoint) ->
        {:invalid,
         "delayed_payment_basepoint (#{inspect(msg.delayed_payment_basepoint)}) is not a valid public key"}

      !Key.valid_public_key?(msg.htlc_basepoint) ->
        {:invalid, "htlc_basepoint (#{inspect(msg.htlc_basepoint)}) is not a valid public key"}

      !Key.valid_public_key?(msg.first_per_commitment_point) ->
        {:invalid,
         "first_per_commitment_point (#{inspect(msg.first_per_commitment_point)}) is not a valid public key"}
