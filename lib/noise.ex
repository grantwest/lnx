...
defmodule Lnx.Noise do
  alias Lnx.Key
  alias Lnx.NoiseError
  ...
  def new(role, remote_pub_key, local_key, opts) do
    ...
    h =
      case role do
        :initiator -> sha256(h, Key.compress(remote_pub_key))
        :responder -> sha256(h, Key.compress(local_key[:pub]))
      end
    ...
    {:ok,
     ...
     |> Map.put(:h, h)
     |> Map.put(:ck, ck)
     |> Map.put(:remote_pub_key, Key.decompress(remote_pub_key))
  ...
  def new(:responder, local_key, opts) do
    ck = :crypto.hash(:sha256, @protocol_name)
    h = :crypto.hash(:sha256, ck <> @prologue)
    h = sha256(h, Key.compress(local_key[:pub]))
  ...
  defp ecdh(rk, k) do
    ...
    :crypto.hash(:sha256, Key.compress(pub))
  end
  ...
  def act1(%{role: :initiator, h: h, ck: ck, remote_pub_key: rpk} = state) do
    e = generate_e(state)
    h = sha256(h, Key.compress(e[:pub]))
    ss = ecdh(rpk, e[:priv])
    {ck, temp_k1} = hkdf(ck, ss)
    c = encrypt_with_ad(temp_k1, 0, h, <<>>)
    h = sha256(h, c)
    payload = <<0>> <> Key.compress(e[:pub]) <> c
  ...
  def act3(%{role: :initiator, h: h, ck: ck, re: re, local_key: s, temp: temp_k2} = state) do
    c = encrypt_with_ad(temp_k2, 1, h, Key.compress(s[:pub]))
    h = sha256(h, c)
    ss = ecdh(re, s[:priv])
  ...
  defp parse(input),
    do:
      raise(NoiseError,
        type: :parsing_error,
        message: "incorrect number of bytes, expected 50 or 66, got #{byte_size(input)}"
      )

  defp check_version(0), do: true

  defp check_version(v),
    do: raise(NoiseError, type: :unsupported_version, message: "expected version 0, but got #{v}")

  defp check_public_key(key) do
    unless Key.valid_public_key?(key) do
